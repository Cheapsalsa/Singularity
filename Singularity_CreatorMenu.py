...
class World(object):
    ...
    def render(self, surface):
        ...
        for c1 in range(0, self.width, int(self.width / 25)):
            pygame.draw.line(surface, (63, 63, 63), (c1, 0), (c1, self.height), 1)
        for r1 in range(0, self.height, int(self.height / 15)):
            pygame.draw.line(surface, (63, 63, 63), (0, r1), (self.width, r1), 1)
...
splash2 = font.render("PRESS 'SELECT' FOR GAME OPTIONS", 1, (255, 255, 255))
...
player_list[0].ready = True
player_list[0].active = True
player_list[0].color = (0, 255, 0)
player_list[1].ready = True
player_list[1].active = True
player_list[1].color = (255, 0, 0)
player_list[2].ready = True
player_list[2].active = True
player_list[2].color = (45, 120, 255)
player_list[3].ready = True
player_list[3].active = True
player_list[3].color = (255, 255, 0)
...
while in_game:
    ...
    while menu_stage:
        ...
        if keyFuncs.key_is_pressed('r'):
            while (keyFuncs.key_is_pressed('r')):
                continue
            projectile_list.remove(projectile_list[-1])
        if keyFuncs.key_is_pressed('g'):
            while (keyFuncs.key_is_pressed('g')):
                continue
            shooty = not shooty
        if keyFuncs.key_is_pressed('space'):
            while (keyFuncs.key_is_pressed('space')):
                continue
            pos = pygame.mouse.get_pos()
            projectile_list.append(
                    Projectile(random.randint(2, 10), ssize, pos[0], pos[1], (200, 200, 200)))
        if keyFuncs.key_is_pressed('d'):
            while (keyFuncs.key_is_pressed('d')):
                continue
            ssize += 5
        if keyFuncs.key_is_pressed('a'):
            while (keyFuncs.key_is_pressed('a')):
                continue
            ssize -= 5
        ...
        screen.blit(splash2, (screen_width / 2 - 150, screen_height - 158))
       ...
       # screen.blit(menu_font.render(str(pygame.mouse.get_pos()), 1, (255, 255, 255)), (pygame.mouse.get_pos()))
